    ...
    switch (paramType) {
      ...
      default:
        return {
          ...
          height: -this.z, // Add height up to 3m
          altitude: -this.z, // Add altitude up to 3m
        }
    }
  }
}

export function ParameterCard({ paramType }: ParameterCardProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          const mockData = simulator.current.getParameterData(paramType)
          setParamData(mockData)
          setError(null)
        } else {
        ...
        console.error(`Error fetching ${paramType}:`, err)
        // Generate realistic mock data if fetch fails
        const mockData = simulator.current.getParameterData(paramType)
        setParamData(mockData)
        setError(null)
